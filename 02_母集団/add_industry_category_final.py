...
def main():
    ...
    industry_df = pd.read_csv(industry_file, delimiter='\t')
    
    # NoをString型として扱う（N/Aと数値を同じ型で扱うため）
    industry_df['No'] = industry_df['No'].astype(str)
    ...
    for _, row in unlisted_companies.iterrows():
        # 売上高を億円に変換（円から億円へ）
        sales_str = row.get('売上高(2022年時点)', '')
        sales_value = np.nan
        if isinstance(sales_str, str) and sales_str.startswith('¥'):
            try:
                # ¥記号とカンマを除去して数値に変換
                sales_yen = float(sales_str.replace('¥', '').replace(',', ''))
                # 億円に変換
                sales_value = sales_yen / 100_000_000
            except:
                sales_value = np.nan
    ...
    # 元のファイルをバックアップして、新しいファイルで置き換え
    import shutil
    shutil.copy(financial_file, financial_file + '.backup')
    shutil.move(output_file, financial_file)
    print(f"元のファイルをバックアップし、{financial_file} を更新しました")
    ...
    if len(no_industry) > 0:
        ...
        print("サンプル:")
        print(no_industry[['コード', '企業名']].head(10))
